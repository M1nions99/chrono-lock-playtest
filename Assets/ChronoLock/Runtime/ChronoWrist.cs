...
namespace ChronoLock
{
    ...
    public sealed class ChronoWrist : MonoBehaviour
    {
        const int ArmLayer=30;
        public ChronoGame game;
        public bool IsVisible => presentation && presentation.gameObject.activeInHierarchy;
        public bool IsOpeningPresentation => IsVisible && game && game.Opening && game.Opening.Running;
        ...
        TemporalDevice lastActive;
        TemporalMode lastMode;
        bool wasPlaying;
        ...
        void Start()
        {
            ...
            if(!game)game=FindFirstObjectByType<ChronoGame>();
            ...
            armCamera.transform.SetParent(transform,false);armCamera.cullingMask=1<<ArmLayer;
        ...
        void LateUpdate()
        {
            ...
            bool opening=game.Opening&&game.Opening.Running;
            bool openingVisible=opening&&!game.Paused&&game.Interface&&game.Interface.Page==ChronoPage.Awakening&&game.Opening.Elapsed>=2.7f&&game.Opening.Elapsed<7.8f;
            bool playing=game.IsPlaying&&game.Interface&&game.Interface.Page==ChronoPage.HUD;
            bool visible=openingVisible||playing;
            presentation.gameObject.SetActive(visible);armCamera.enabled=visible;
            if(!visible){if(!game.HasStarted||game.Completed){gesture=castPulse=releasePulse=0;lastActive=null;}wasPlaying=false;return;}
            ...
            if(!game.Paused)motionClock+=dt;
            var active=game.ActiveDevice;
            if(active&&(active!=lastActive||game.SelectedMode!=lastMode))castPulse=1;
            if(!active&&lastActive)releasePulse=1;
            lastActive=active;lastMode=game.SelectedMode;
            castPulse=Mathf.MoveTowards(castPulse,0,dt*2.8f);releasePulse=Mathf.MoveTowards(releasePulse,0,dt*3.5f);
            float desired=active?1:0;
            gesture=motion?Mathf.MoveTowards(gesture,desired,dt*4.5f):desired;
            Vector3 position;Quaternion rotation;float curl;
            if(openingVisible)
            {
                float t=game.Opening.Elapsed;
            ...
            {
                if(!wasPlaying&&active==null){gesture=castPulse=releasePulse=0;}
            ...
            GlyphColor=openingVisible?new Color(.08f,.8f,.85f):game.SelectedMode==TemporalMode.Accelerate?new Color(1,.40f,.055f):game.SelectedMode==TemporalMode.Rewind?new Color(.62f,.25f,1):new Color(.035f,.8f,1);
